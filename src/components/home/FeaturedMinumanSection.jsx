...
export default function FeaturedMinumanSection({ featuredMinuman, favorites, toggleFavorite, onSelectRecipe }) {
  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-6 md:mb-8">
        <h2 className="text-xl md:text-3xl font-bold text-slate-800">Minuman Populer</h2>
          ...
          return (
            <div 
              ...
              className={`group transform transition-all duration-700 ${
                visibleMinuman.has(index) ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'
              }`}
            >
              <div className="relative bg-white/15 backdrop-blur-xl border border-white/25 rounded-2xl md:rounded-3xl overflow-hidden shadow-lg md:shadow-2xl shadow-indigo-500/5 hover:shadow-indigo-500/15 transition-all duration-500 group-hover:scale-105 group-hover:bg-white/20">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
