...
export default function MakananPage({ allRecipes, favorites, toggleFavorite, onSelectRecipe }) {
    const [searchQuery, setSearchQuery] = useState('');
    const [filteredRecipes, setFilteredRecipes] = useState([]);
    const [currentPage, setCurrentPage] = useState(1);
    const itemsPerPage = 3;

    // Filter hanya resep makanan
    const foodRecipes = allRecipes.filter(recipe => recipe.category === 'makanan');
            ...
            <main className="max-w-7xl mx-auto px-4 md:px-8 py-8 md:py-12">
                ...
                <div className="text-center mb-8">
                    ...
                    <p className="text-slate-600 mt-4">
                        Menampilkan {filteredRecipes.length} resep makanan
                    </p>
                </div>
                ...
                {currentRecipes.length > 0 ? (
                    ...
                    <div className="text-center py-12">
                        <p className="text-slate-500 text-lg">
                            Tidak ada resep makanan yang sesuai dengan pencarian Anda
                        </p>
                    </div>
                )}
                ...
                {totalPages > 1 && (
                    <div className="flex justify-center items-center mt-12 space-x-4">
                        <button
                            ...
                            className="px-6 py-2 bg-white border-2 border-blue-500 text-blue-500 rounded-lg font-medium hover:bg-blue-500 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-white disabled:hover:text-blue-500 transition"
                        >
                            Sebelumnya
                        </button>
